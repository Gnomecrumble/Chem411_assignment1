...
#     print('distance', distance, "cm")
   # lcd.clear() # clear the screen
    #dmin = min(distance) #save min and max of distance
    #dmax = max(distance)
    ...
    #if distance<200: #distance under which no alarm
      # buzzer.value(1)
    #else:   #alarm sounds over 200cm
     #   buzzer.value(0) #turn on buzzer
     #   time.sleep(0.05) #leave buzzer on for quick chirp then shutoff for sanity sake
     #   buzzer.value(1)
    time.sleep(1)